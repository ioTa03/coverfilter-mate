...
import { FilterCriteria, MovementType, Location, ProjectType, LoadingType, JointPlacement, SpecialRequirement, BuildingType } from '@/types';
...
const FilterForm = ({ onFilterSubmit }: FilterFormProps) => {
  ...
  const completedSteps = useMemo(() => {
    ...
    // Step 3: Project Type and Maximum Loading
    if (criteria.projectType !== null || criteria.maxLoadings.length > 0) {
      completed.add(3);
    }
    
    // Step 4: Joint Placement and Special Requirements
    if (criteria.jointPlacements.length > 0 || criteria.specialRequirements.length > 0) {
      completed.add(4);
    }
  ...
  const getClearMessage = () => {
    return `Clear ${
      ...
      currentStep === 3 ? "Project & Loading" :
      currentStep === 4 ? "Placement & Requirements" :
      "Building Type"
    } Filters`;
  };
  ...
  const projectTypes: ProjectType[] = ['New Construction', 'Retrofit'];
  ...
  const buildingTypes: BuildingType[] = ['Commercial', 'Residential', 'Industrial', 'Hospitality', 'Healthcare', 'Stadium', 'Metro', 'Airport', 'Mixed Use'];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="space-y-6">
        ...
        <FilterStep
          title="Joint Width"
          description="Specify the width parameters for your expansion joint"
        ...
        <FilterStep
          ...
          description="Specify project type and maximum loading requirements"
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div>
              <Label className="text-base font-medium mb-2 block">Type of Project</Label>
              <RadioGroup 
                value={criteria.projectType || ''} 
                onValueChange={(value) => 
                  updateCriteria({ projectType: value as ProjectType || null })
                }
              >
                {projectTypes.map((type) => (
                  <div key={type} className="flex items-center space-x-2">
                    <RadioGroupItem value={type} id={`project-${type}`} />
        ...
        <FilterStep
          ...
          description="Select joint placement and special requirements"
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            ...
            <div>
              <Label className="text-base font-medium mb-2 block">Special Requirements</Label>
              <div className="grid grid-cols-1 gap-2">
